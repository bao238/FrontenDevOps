...
export default function App() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    localStorage.setItem("techstore_user", JSON.stringify(loggedUser));
    setCustomerInfo((prev) => ({
      ...prev,
  ...
  const handleQuickDemoLogin = () => {
    ...
    localStorage.setItem("techstore_user", JSON.stringify(demoUser));
    setCustomerInfo((prev) => ({
      ...prev,
  ...
  const handleApplyCoupon = (e: React.FormEvent) => {
    ...
    if (promoCode.trim().toUpperCase() === "DEVOPS10") {
      ...
      showToast("Áp dụng mã DEVOPS10 thành công! Giảm 10%");
    } else {
  ...
  const handleCheckoutSubmit = (e: React.FormEvent) => {
    ...
    const generatedId = "TECH-" + Math.floor(100000 + Math.random() * 900000);
    ...
    try {
      localStorage.setItem("techstore_orders", JSON.stringify(updatedOrders));
    } catch {
      // ignore
    }
  ...
  return (
    <div className="min-h-screen flex flex-col justify-between">
      <div className="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-6 space-y-10">
        {/* Toast Notification */}
        {toastMessage && (
        ...
        <header className="bg-white rounded-2xl p-4 sm:p-5 border border-slate-200 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
          {/* Brand */}
          <div className="flex items-center space-x-3 w-full md:w-auto justify-between">
            <div className="flex items-center space-x-3">
              <div className="w-11 h-11 rounded-2xl bg-gradient-to-tr from-blue-600 via-indigo-600 to-sky-500 flex items-center justify-center text-white shadow-lg shadow-blue-500/25">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 font-medium mt-1">
                  React SPA &middot; Thiết Bị Công Nghệ Chính Hãng
                </p>
              </div>
            </div>
        ...
        <section className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-blue-700 via-indigo-700 to-sky-600 text-white p-6 sm:p-10 shadow-xl">
          ...
          <div className="relative z-10 max-w-2xl space-y-4">
            <div className="inline-flex items-center space-x-2 bg-white/20 backdrop-blur-md px-3.5 py-1 rounded-full text-xs font-semibold text-white">
              ...
              <span>Siêu Sale React Frontend 2024</span>
            </div>
            <h2 className="text-2xl sm:text-4xl font-extrabold tracking-tight leading-tight">
              Nâng Tầm Trải Nghiệm Công Nghệ & Lập Trình
            </h2>
            <p className="text-blue-100 text-sm sm:text-base leading-relaxed">
              Giảm đến 30% cho các dòng MacBook, iPhone, bàn phím cơ và thiết bị âm thanh đỉnh cao. Đóng gói với Docker & Nginx siêu nhẹ.
            </p>
            <div className="pt-2 flex flex-wrap items-center gap-3">
              ...
              <div className="text-xs text-blue-200 flex items-center space-x-1.5">
                ...
                <span>Mã giảm 10%: <strong className="text-white font-mono font-bold">DEVOPS10</strong></span>
              </div>
            </div>
          </div>
        </section>
          ...
          <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div className="flex items-center space-x-2 w-full md:w-auto justify-end">
              ...
              <select
                ...
                className="bg-slate-50 border border-slate-200 text-slate-700 text-xs font-semibold rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer"
              >
        ...
        {/* DevOps CI/CD Infrastructure Information Card */}
        <section className="bg-slate-900 text-white rounded-3xl p-6 sm:p-8 border border-slate-800 shadow-xl relative overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="space-y-2">
              <div className="inline-flex items-center space-x-2 text-xs font-semibold text-blue-400 bg-blue-950 px-3 py-1 rounded-full border border-blue-800">
                <ShieldCheck className="w-3.5 h-3.5" />
                <span>Hạ Tầng React SPA &amp; DevOps CI/CD</span>
              </div>
              <h3 className="text-xl sm:text-2xl font-bold">
                React + Vite Đóng Gói Docker &amp; Nginx Triển Khai Render
              </h3>
              <p className="text-xs sm:text-sm text-slate-400 max-w-2xl leading-relaxed">
                Mã nguồn React SPA được tối ưu hóa build tĩnh (Static build), phục vụ bởi Nginx web server hiệu năng cao, tự động hóa CI/CD qua GitHub Actions.
              </p>
            </div>
            <div className="flex flex-wrap gap-2 text-xs font-mono">
              <div className="bg-slate-800/80 border border-slate-700 px-3 py-2 rounded-xl">
                <span className="text-slate-400">Stack:</span>{" "}
                <span className="text-sky-400 font-bold">React 18 + Vite</span>
              </div>
              <div className="bg-slate-800/80 border border-slate-700 px-3 py-2 rounded-xl">
                <span className="text-slate-400">Server:</span>{" "}
                <span className="text-emerald-400 font-bold">Nginx Alpine</span>
              </div>
              <div className="bg-slate-800/80 border border-slate-700 px-3 py-2 rounded-xl">
                <span className="text-slate-400">Cloud:</span>{" "}
                <span className="text-purple-400 font-bold">Render.com</span>
              </div>
            </div>
          </div>
        </section>
                ...
                {authTab === "login" ? (
                  ...
                  <form onSubmit={handleRegister} className="space-y-3 text-xs">
                    <div className="text-center mb-2">
                      ...
                      <p className="text-xs text-slate-500">
                        Tham gia cùng hàng ngàn khách hàng của TechStore
                      </p>
                    </div>
                ...
                <div className="p-5 border-t border-slate-200 bg-white space-y-4">
                  <form onSubmit={handleApplyCoupon} className="flex space-x-2">
                    <input
                      ...
                      placeholder="Mã giảm giá (VD: DEVOPS10)"
              ...
              {!orderSuccess ? (
                ...
                <div className="text-center py-6 space-y-4">
                  ...
                  <p className="text-xs text-slate-500 max-w-sm mx-auto">
                    ...
                    đã mua sắm tại TechStore. Đơn hàng của bạn đang được xử lý.
                  </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-8">
            ...
            <div className="space-y-3">
              ...
              <div className="flex flex-wrap gap-1.5">
                <span className="px-2 py-1 bg-sky-50 text-sky-700 rounded text-[11px] font-mono border border-sky-200">React 18</span>
                <span className="px-2 py-1 bg-blue-50 text-blue-700 rounded text-[11px] font-mono border border-blue-200">Docker</span>
                <span className="px-2 py-1 bg-emerald-50 text-emerald-700 rounded text-[11px] font-mono border border-emerald-200">Nginx Alpine</span>
                <span className="px-2 py-1 bg-purple-50 text-purple-700 rounded text-[11px] font-mono border border-purple-200">Render Cloud</span>
              </div>
              <p className="text-[11px] text-slate-400">
                Containerized via multi-stage Docker &amp; Nginx web server.
              </p>
            </div>
          </div>
